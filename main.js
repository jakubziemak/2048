...
class Game {
    ...
    updateGameBoard = (cell, target, summed = false) => {
        ...
        if(summed){
            const [tileOne, tileTwo] = this.tiles.filter(({ x, y }) => x == targetX && y == targetY)

            this.tilesToDelete.push(tileOne.id)
            this.tilesToDelete.push(tileTwo.id)

            this.tilesToCreate.push({val: val, y: targetY, x: targetX, id: this.newId()})
        }
    }
    ...
    deleteTiles = () => {
        ...
        .then(() => {
            ...
            this.tiles = this.tiles.filter(tile => !this.tilesToDelete.includes(tile.id))
